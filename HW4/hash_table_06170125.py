...
class MyHashSet:
    ...
    def add(self, key): #新增
        ...
        if self.contains(key) != True: #如果數值不存在
            if self.val[ind] == None: #如果list沒東西，也就是加進來的東西為第一個
                self.val[ind] = new #那他就是新的
    ...
    def remove(self, key): #刪除
        ind = self.md5(key) #經過md5函數的數值
        num = self.val[ind] #這是節點
        none = None
        
        if self.contains(key) == False: #沒數值
            return False #直接回傳
        #if self.contains(key) != False:
        if num != None and key != num.val:
            none = num
            num = num.next
        if none == None:
                self.val[ind] = self.val[ind].next
        else:
            none.next = num.next
...
#             print("c1")
            return False
        if num.val == key: #如果查詢東西剛好等於第一數
            return True
        if num.next: #如果查詢的東西是節點
            while (num.next) != None: #下一位不適空值
                num=num.next #往下一位繼續查
                if num.val == key: #查到就true
                    return True
                if num.val != key: #查不到就false
                    False
        else:
            return False   
